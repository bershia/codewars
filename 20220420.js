// 1. In a factory a printer prints labels for boxes. For one kind of boxes the printer has to use 
...
// printer as a string representing a rational whose numerator is the number of errors and 
// the denominator the length of the control string. Don't reduce this fraction to a simpler expression.

// The string has a length greater or equal to one and contains only letters from ato z.

function printerError(s) {
    let badS = 'nopqrstuvwxyz'
    let numOfErr = 0
    for(let i = 0; i < s.length; i++){
      badS.includes(s[i]) ? numOfErr++ : numOfErr
    }
    return `${numOfErr}/${s.length}`
}